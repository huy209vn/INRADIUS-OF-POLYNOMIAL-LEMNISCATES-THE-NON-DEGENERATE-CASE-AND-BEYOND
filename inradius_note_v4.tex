...
(if $U$ had a bounded complementary component $H$, then
$\log\abs{P}$ would be harmonic on $H$ with boundary value $0$,
forcing $\log\abs{P} \equiv 0$ on $H$, a contradiction).
...
\[
  B_a'(w) \;=\; \frac{w\bigl(-2a + (3+a^2)w - 2aw^2\bigr)}{(1-aw)^2}.
\]
The point $w_c = 0$ is a critical point with $B_a''(0) = -2a$.
The nonzero critical point $w_c'$ is the small root of the quadratic
$-2a + (3+a^2)w - 2aw^2 = 0$; for small $a$, this satisfies
$w_c' = 2a/3 + O(a^3)$.  A direct evaluation then gives
$\abs{B_a''(w_c')}(1-\abs{w_c'}^2)^2 = 2a + O(a^3)$.
...
multiplicity $2$, and Lemma~\ref{lem:local-multiplicity}
